...
- Daten einer Stichprobe zusammenfassend beschreiben. Keine Verallgemeinerung der Ergebnisse.
...
![](../images/population-stichprobe.png){fig-align="center"}

::: {.notes}
- Um was geht es im Modul im Allgemeinen
- Wo ist die deskriptive Statistik verortet?
- Bsp. Population: Körpergrösse von Erwachsenen in der CH.
:::
...
- Konkrete Werte von Beobachtungsmerkmen (z.B. Körpergrösse = 170 cm)
...
::: {.notes}
- Es gibt auch andere Merkmalstypen als die Körpergrösse.
- Für Analyse muss der Merkmalstyp berücksichtigt werden.
:::
...
- Es gibt auch andere Merkmalstypen als die Körpergrösse.
- Für Analyse muss der Merkmalstyp berücksichtigt werden.
- Grobe Unterteilung in qualitativ und quantitativ
:::
...
::: {.notes}
- Unterscheiden nach Kennzahlen und grafischen Methoden
:::
...
::: {.notes}
- Beschrieb der zentralen Tendenz eines Merkmals
- Beschrieb von Teilmengen und Anteilen: wie gross sind die 10% grössten Erwachsenen
:::
...
::: {.notes}
- typischer Wert in der Mitte oder durchschnittlicher Wert?
- Verwendung von Mittelwert oder Median hängt von der Verteilung der Daten ab
- Welche Auswertungsmethoden sollen weiter verwendet werden? (parametrisch/nicht-parametrisch)
:::
...
ggplot(data, aes(x = x, y = y, color = sample)) +
...
- Trotz gleichem Mittelwert kann nicht behauptet werden, dass die Stichproben aus der gleichen Verteilung stammen.
- Beobachtungen der Stichprobe *B* streuen mehr.
- Kennzahlen der Streuung quantifizieren diese Eigenschaft.

::: {.notes}
- Mittelwert erklärt die Daten von *A* besser, weil die Werte näher am Mittelwert liegen.
:::
...
- Visuell können wir rascher Tatsachen auffassen, als aus Kennzahlen und komplexen Tabellen
- Muster erkennen und kommunizieren
- Rasch Verteilungen beurteilen und Stichproben leicht miteinander vergleichen.
:::
...
- Anzahl Klassen $n_k \approx \sqrt{n}$
- Richtwert Anzahl Klassen: 5 bis 20 > Sinnvoll ab $n = 40$

::: {.notes}
- Beurteilung
  - Variabilität der Daten (z.B. Streubreite)
  - Am meisten vorkommende Werte
  - Symmetrie
- Klassenbreite ist elementar fürs Erscheinungsbild
- Anzahl Klassen $\sqrt{n}$. Richtwert 5 bis 20 Klassen > ca. ab $n=50$
- Zu schmale Klassen: fehlende Balken
- Zu breite Balken: Verlust an Details
- Keine Lücken zwischen Balken (x-Achse Kontinuum)
:::
...
::: {.notes}
- Wichtige Darstellungsmöglichkeit für quantitative Merkmale: Verteilungen visualisieren, Gruppen oder Zeitpunkte vergleichen, Auffällige Werte entdecken.
- Beurteilung:
  - Spannweite
  - Lage der zentralen 50% der Daten (IQR)
  - Lage des Medians
  - Verteilung der Daten
- Symmetrie der Daten: Median in der Mitte der Box
...
ggplot(df.stud, mapping = aes(x = Geschlecht, y = Groesse)) +
  # geom_boxplot(outlier.shape = 8, outlier.size = 3, outlier.color = "red") +
  geom_boxplot() +
  ## geom_jitter(position = position_jitter(0.2), size = 1) +
  labs(y = "Körpergrösse (cm)") +
  theme(text = element_text(size = 22))
```
...
::: {.notes}
- Alter von 11, resp. 13 Personen (rechts)
- Linke Seite Daten ohne Ausreisser
- Rechte Seite Daten mit Ausreissern
- Median und IQR bleiben gleich/ähnlich
- Mittelwert und SD werden grösser
:::
...
::: {.notes}
- Kreuztabelle und Balkendiagramm als Methoden der deskriptiven Statistik
:::
...
::: {.notes}
- Links: Ausschnitt der ersten 7 Beobachtugnsheiten
- Rechts: Kreuztabelle
...
::: {.notes}
- Absolute Anzahl in der Gruppe
- Relative Häufigkeit in der Gruppe
:::
...
::: {.notes}
- Häufig sieht man Tortendiagramme in "eifachen" Infografiken
- Menschliches Auge kann Unterschiede von Kreissektoren und Flächen schlecht beurteilen
- In den Tortendiagrammen sind im Verleich zu den Balkendiagrammen die Unterschiede schlecht sichtbar
- Kleine Anteile in Tortendiagrammen schlecht zu erkennen
:::
